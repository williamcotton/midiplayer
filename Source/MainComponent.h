...
// Synthesizer voice (leave these if you plan to use them later)
...
//-----------------------------------------------------------------------
// Modified MainComponent: Notice that we no longer inherit from
// juce::AudioSource.
...
public:
  ...
  void processSegment(const juce::AudioSourceChannelInfo &bufferInfo,
                      int segmentStartSample, int segmentNumSamples,
                      double segmentStartBeat, double segmentEndBeat);
  void reTriggerSustainedNotesAt(double loopStartBeat);

  double convertTicksToBeats(double ticks) const { return ticks / 480.0; }
  double convertBeatsToTicks(double beats) const { return beats * 480.0; }
